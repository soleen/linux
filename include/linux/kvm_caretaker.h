 ...
 * @cb: Host copy of the Caretaker control block.
 * @job: On-Core job this vCPU is scheduled as, or NULL.
 *
 * @cb is a SECOND, INDEPENDENT instance of the control block, not a pointer to
 * the one in the KHO-preserved page.  The two deliberately live in different
 * address spaces: this one is ordinary host memory owned by struct kvm_vcpu,
 * while the preserved copy is what the orphaned core executes against after
 * the kexec.  Pointing one at the other would be precisely the
 * stale-pointer-across-kexec bug this series exists to avoid.
 *
 * Reusing the type (rather than redeclaring its seven fields) is what makes
 * the paired updates in virt/kvm/caretaker.c visibly deliberate.
 */
...
/*
 * Thin wrappers around the four helpers above, operating on the host copy of
 * the control block that lives inside struct kvm_vcpu.
 *
 * These are out-of-line on purpose.  They dereference vcpu->caretaker, so they
 * need struct kvm_vcpu to be complete -- but this header is included from
 * kvm_host.h:47, i.e. before that definition exists.  Keeping them here as
 * declarations is what lets kvm_host.h stay free of Caretaker bodies.  The
 * only caller on a warm path is the KVM_RUN *ioctl* entry, which already costs
 * a syscall, so the call is not measurable.
 */
...
static inline void kvm_caretaker_vcpu_init(struct kvm_vcpu *vcpu, int vcpu_id) {}
static inline bool kvm_caretaker_vcpu_is_attached(struct kvm_vcpu *vcpu)
{
	return true;
}
static inline void kvm_caretaker_vcpu_detach(struct kvm_vcpu *vcpu) {}
static inline void kvm_caretaker_vcpu_attach(struct kvm_vcpu *vcpu) {}
