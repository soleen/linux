...
/* Scheduler timeout and default quantum */
#define CARETAKER_DEFAULT_QUANTUM_MS	10
...
static inline struct caretaker_session *
caretaker_get_session(struct liveupdate_session *s)
{
	return NULL;
}
static inline int caretaker_alloc_session_pgd(struct caretaker_session *sess) { return 0; }
static inline void caretaker_free_session_pgd(struct caretaker_session *sess) {}
static inline int caretaker_map_session_range(struct caretaker_session *sess,
					      phys_addr_t pa, unsigned long va,
					      size_t size, pgprot_t prot) { return 0; }
static inline void arch_caretaker_flush_tlb(struct caretaker_session *sess) {}
static inline int arch_caretaker_init_session_pgd(struct caretaker_session *sess,
						  struct trans_pgd_info *info) { return 0; }
...
{
	return IS_ENABLED(CONFIG_CARETAKER) && cpu_is_preserved(cpu);
}

static inline bool caretaker_is_enabled(void)
{
	return IS_ENABLED(CONFIG_CARETAKER);
}
