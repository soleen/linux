 ...
 *    KHO-preserved vCPU architectural state pages (struct caretaker_x86_page)
 *    holding guest/host registers, VMCS/VMCB pointers, and control areas.
 *
 * 5. Hardware Control MMIO (Conditional):
 *    Local APIC base (fix_to_virt(FIX_APIC_BASE)) mapped as PAGE_KERNEL_IO
 *    only if operating in legacy xAPIC MMIO mode (!x2apic_mode). In modern
 *    x2APIC MSR mode, no MMIO mapping is created.
 *
