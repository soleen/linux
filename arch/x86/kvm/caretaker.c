...
void kvm_x86_caretaker_signal_attach(struct kvm_vcpu *vcpu, u64 cb_pa)
{
	...
	if (cb_pa) {
		struct kvm_caretaker_cb *cb = phys_to_virt(__sme_clr(cb_pa));
			   ...
			   struct kvm_vcpu *vcpu, u64 deadline_ticks)
{
	...
	if (reason != ONCORE_EXIT_ATTACH_SIGNALED &&
	    vcpu && READ_ONCE(cxp->cb.attachment_state) == KVM_CARETAKER_DETACHED &&
...
__caretaker_text void kvm_x86_caretaker_arm_timer(u64 deadline_ticks)
{
	...
	if (caretaker_x86_has_tsc_deadline) {
		...
		if (global_oncore_sched_config.tsc_khz && caretaker_x86_lapic_timer_period) {
			u64 period = caretaker_x86_lapic_timer_period;
			u64 apic_khz = (period * HZ) / 1000ULL;

			count = (delta_tsc * apic_khz) / ((u64)global_oncore_sched_config.tsc_khz);
		} else {
			count = delta_tsc >> 4;
		}
		if (count == 0)
...
{
	if (ser->cb.phys) {
		struct kvm_caretaker_cb *cb = phys_to_virt(__sme_clr(ser->cb.phys));
