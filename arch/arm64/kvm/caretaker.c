				 ...
				 struct kvm_vcpu_luo_ser *ser)
{
	...
	struct oncore_session *sess = vcpu->caretaker_job ?
...

	{
		...
		if (vcpu->vcpu_id == 0 || !active_vm) {
			...
			if (vcpu->kvm->created_vcpus) {
				max_vcpus = max_t(unsigned int,
						  vcpu->kvm->created_vcpus,
						  KVM_MAX_VCPUS);
			}

			if (!active_vm) {
				size_t sz = struct_size(active_vm, vcpus,
							max_vcpus);

				active_vm = kho_alloc_preserve(sz);
				if (!IS_ERR(active_vm)) {
					memset(active_vm, 0, sz);
					active_vm->max_vcpus = max_vcpus;
					oncore_session_map_buffer(sess,
								  active_vm,
								  sz);
				}
			}
...
arm64_caretaker_op_decode_exit(void *data, struct kvm_caretaker_exit *exit)
{
	...
	if (ARM_EXCEPTION_CODE(ret) == ARM_EXCEPTION_IRQ) {
		...
		if (iar1 < GIC_SPECIAL_INTID_START) {
	...
	if (ARM_EXCEPTION_IS_TRAP(ret)) {
		...
		if (ec == ESR_ELx_EC_DABT_LOW) {
			...
			if (hpfar & HPFAR_EL2_NS) {
				fault_ipa = (FIELD_GET(HPFAR_EL2_FIPA, hpfar) << PAGE_SHIFT) |
					    (cap->ctx.fault.far_el2 & ~PAGE_MASK);
			} else {
				fault_ipa = 0;
			}

			if ((fault_ipa & PAGE_MASK) == ARM_VIRT_PL011_BASE &&
			    (esr & ESR_ELx_ISV)) {
				...
				exit->mmio_io.val_ptr =
					(rt < 31) ? &cap->ctx.ctxt.regs.regs[rt] : NULL;
				return;
			}
		}
...
arm64_caretaker_emulate_pl011(struct caretaker_arm64_page *cap,
...
STACK_FRAME_NON_STANDARD(arm64_caretaker_emulate_pl011);
...
arm64_caretaker_op_handle_exit(void *data, struct kvm_caretaker_exit *exit)
{
	...
	if (exit->type == KVM_CARETAKER_EXIT_CONSOLE) {
		u64 *target = exit->mmio_io.val_ptr ?
			      exit->mmio_io.val_ptr : &exit->mmio_io.val;

		handled = arm64_caretaker_emulate_pl011(cap, exit->mmio_io.addr,
	...
	if (exit->type == KVM_CARETAKER_EXIT_ARCH) {
		...
		if (ec == ESR_ELx_EC_DABT_LOW) {
			...
			if ((fault_ipa >= ARM_VIRT_GIC_DIST_BASE &&
			     fault_ipa < ARM_VIRT_GIC_REDIST_END) &&
			    (esr & ESR_ELx_ISV) && !(esr & ESR_ELx_WNR)) {
				u32 rt = (esr & ESR_ELx_SRT_MASK) >> ESR_ELx_SRT_SHIFT;
				u64 data_val = 0;

				if (fault_ipa == (ARM_VIRT_GIC_DIST_BASE + GICD_CTLR))
					data_val = GICD_CTLR_ENABLE_G1A | GICD_CTLR_ENABLE_G1;
				else if ((fault_ipa & (GICR_FRAME_SIZE - 1)) == GICR_WAKER)
					data_val = 0;
...
kvm_arch_vcpu_caretaker_run(void *data, u64 deadline_ticks)
{
	...
	if (data) {
		...
		if (cb->runtime_size == sizeof(struct caretaker_arm64_page) &&
		    cb->attachment_state <= KVM_CARETAKER_ATTACHING) {
					...
					struct kvm_vcpu_luo_ser *ser)
{
	if ((ser->flags & KVM_VCPU_LUO_FLAG_CARETAKER) && ser->cb.phys) {
		struct kvm_caretaker_cb *old_cb = phys_to_virt(ser->cb.phys);
		struct caretaker_arm64_page *cap =
			container_of(old_cb, struct caretaker_arm64_page, cb);
