				   ...
				   void *arch_data)
{
	...
	pcpu = (vcpu->caretaker.job && vcpu->caretaker.job->assigned_cpu >= 0) ?
	       vcpu->caretaker.job->assigned_cpu :
	       (vcpu->cpu >= 0 && vcpu->cpu < nr_cpu_ids ? vcpu->cpu : 0);

	cvcpu->cb.attachment_state = KVM_CARETAKER_ATTACHED;
