				   ...
				   void *arch_data)
{
	...
	pcpu = (vcpu->cb.pcpu_id != KVM_CARETAKER_INVALID_PCPU &&
...
kvm_caretaker_vcpu_run(struct kvm_caretaker_vcpu *cvcpu, u64 deadline_ticks)
{
	...
	if (READ_ONCE(cvcpu->cb.attachment_state) == KVM_CARETAKER_ATTACHING) {
		WRITE_ONCE(cvcpu->cb.attachment_state, KVM_CARETAKER_ATTACHED);
				    ...
				    struct kvm_caretaker_vcpu *cvcpu)
{
	...
	if (cvcpu)
		pcpu = cvcpu->cb.pcpu_id;
	else if (vcpu->cb.pcpu_id != KVM_CARETAKER_INVALID_PCPU)
				...
				struct kvm_vcpu_luo_ser *ser)
{
	...
	if (target_cpu >= 0) {
		...
		kvm_caretaker_detach(&vcpu->cb);
	} else {
		vcpu->cb.pcpu_id = KVM_CARETAKER_INVALID_PCPU;
	}
	...
	if (target_cpu >= 0) {
		...
		if (job->data) {
			struct kvm_caretaker_cb *cb = job->data;
			struct oncore_session *sess = oncore_get_session(session);

			cb->pcpu_id = target_cpu;
			kvm_caretaker_detach(cb);
			if (cb->runtime_size && cb->runtime_pa) {
				if (sess) {
					oncore_session_map_range(sess,
						cb->runtime_pa,
				   ...
				   struct kvm_vcpu_luo_ser *ser)
{
	...
	if (vcpu->caretaker_job) {
		oncore_session_cancel_job(session, vcpu->caretaker_job);
		vcpu->caretaker_job = NULL;
	}
	vcpu->cb.pcpu_id = KVM_CARETAKER_INVALID_PCPU;
}
