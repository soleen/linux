...
static u64 __cpu_preserved_text
oncore_ticks_to_ns(const struct oncore_sched_config *cfg, u64 ticks)
{
	u64 freq_khz = cfg->counter_freq_hz / 1000ULL;

	if (freq_khz > 0)
		return (ticks * 1000000ULL) / freq_khz;
	return ticks;
}
			 ...
			 struct oncore_sched_config *cfg)
{
	...
	while (!cpu_preserved_should_exit(cpu)) {
		...
		curr->total_runs++;
		curr->total_runtime_ns += oncore_ticks_to_ns(cfg, end_ticks - start_ticks);
