...
static void dma_pte_free_pagetable(struct dmar_domain *domain,
				   ...
				   int retain_level)
{
	iommu_pt_read_lock(domain->pgd);
	dma_pte_clear_range(domain, start_pfn, last_pfn);

	/* We don't need lock here; nobody else touches the iova range */
	dma_pte_free_level(domain, agaw_to_level(domain->agaw), retain_level,
			   domain->pgd, 0, start_pfn, last_pfn);
	iommu_pt_read_unlock(domain->pgd);
	...
	do {
		...
		if (start_pfn <= level_pfn &&
			...
			if (freed_level && page_count(npage) == 1) {
				page_ref_inc(npage);
				iommu_pt_read_unlock(domain->pgd);
				iommu_pt_write_lock(domain->pgd);
				if (page_count(npage) == 2) {
					...
					last_pte = pte;
					iommu_pt_add_tail(freelist, npage);
					*freed_level = level;
				}
				iommu_pt_write_unlock(domain->pgd);
				iommu_pt_read_lock(domain->pgd);
				page_ref_dec(npage);
			}
		}
