...
static int intel_iommu_pkram_save_pte(struct pkram_stream *ps, struct dma_pte *pte, int level)
{
	...
	ret = pkram_save_file_page(&pa, virt_to_page(pte));
	if (ret)
		return ret;
	level--;
...
static int intel_iommu_pkram_load_pte(struct pkram_stream *ps, struct dma_pte *pte, int level)
{
	...
	page = pkram_load_file_page(&pa, NULL);
	if (!page)
		return -ENOENT;
	level--;
...
static int save_context_table_cb(struct device_domain_info *info, void *data)
{
	...
	if (!ctx->bus_context_done) {
		ret = pkram_save_file_page(&pa, virt_to_page(context));
		if (ret)
			return ret;
						 ...
						 struct intel_iommu *iommu)
{
	...
	ret = pkram_save_file_page(&pa, virt_to_page(iommu->root_entry));
	if (ret)
		return ret;
					    ...
					    struct intel_iommu *iommu)
{
	...
	ret = pkram_save_file_page(&pa_pages, virt_to_page(iommu->root_entry));
	if (ret)
		return ret;

	ret = pkram_save_file_page(&pa_pages, virt_to_page(iommu->qi->desc));
	if (ret)
		return ret;

	ret = pkram_save_file_page(&pa_pages,
				   virt_to_page(iommu->qi->desc_status));
	if (ret)
		return ret;
...
static int intel_iommu_pkram_save_root_table(struct intel_iommu *iommu)
{
	...
	pkram_prepare_save_obj(&ps, PKRAM_DATA_pages);
