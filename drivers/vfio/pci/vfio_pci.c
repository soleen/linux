				     ...
				     struct keepalive_state *state)
{
	...
	ret = pkram_write(&pa, state, sizeof(*state));
	if (ret < 0)
		return ret;
	ret = pkram_write(&pa, state->vconfig, state->vconfig_size);
	if (ret < 0)
		return ret;
	ret = pkram_write(&pa, state->pci_saved_state,
			       state->saved_state_size);
	if (ret < 0)
		return ret;
	if (!state->pm_save_size)
		goto after_pm_save;
	ret = pkram_write(&pa, state->pm_save, state->pm_save_size);
	if (ret < 0)
		return ret;
after_pm_save:
	ret = pkram_write(&pa, state->saved_irq_data, state->saved_num_ctx * sizeof(void *));
	if (ret < 0)
		return ret;
...
vfio_pci_pkram_load_state(struct pkram_stream *ps)
{
	...
	if (!state)
		return NULL;
	ret = pkram_read(&pa, state, sizeof(*state));
	if (ret < 0)
	...
	if (!state->vconfig)
		goto fail_free_state;
	ret = pkram_read(&pa, state->vconfig, state->vconfig_size);
	if (ret < 0)
	...
	if (!state->pci_saved_state)
		goto fail_free_vconfig;
	ret = pkram_read(&pa, state->pci_saved_state, state->saved_state_size);
	if (ret < 0)
	...
	if (!state->pm_save)
		goto fail_free_saved_state;
	ret = pkram_read(&pa, state->pm_save, state->pm_save_size);
	if (ret < 0)
...
after_pm_save:
	...
	if (!state->saved_irq_data)
		goto fail_free_pm_save;
	ret = pkram_read(&pa, state->saved_irq_data, state->saved_num_ctx * sizeof(void *));
	if (ret < 0)
...
static int vfio_pci_set_keepalive(struct vfio_device *core_vdev,
				  struct vfio_keepalive_data *vka)
{
	struct vfio_pci_device *vdev = container_of(core_vdev,
