...
__caretaker_text static u64 gicv3_caretaker_cpu_to_affinity(int cpu)
{
	...
	if (mpidr == INVALID_HWID && cpu >= 0 &&
	    cpu < ARRAY_SIZE(caretaker_gic.cpu_rdists)) {
		mpidr = cpu;
	}
...
__caretaker_text void __iomem *gicv3_get_rdist_for_cpu(int cpu)
{
	...
	if (cpu >= 0 && cpu < ARRAY_SIZE(caretaker_gic.cpu_rdists) &&
	    caretaker_gic.cpu_rdists[cpu].rdist_base) {
				 ...
				 struct fwnode_handle *handle)
{
	...
	caretaker_gic.nr_redist_regions = min_t(u32, nr_redist_regions,
						ARRAY_SIZE(caretaker_gic.redist_regions));
	if (rdist_regs != caretaker_gic.redist_regions) {
		int reg_idx;

		for (reg_idx = 0; reg_idx < caretaker_gic.nr_redist_regions; reg_idx++)
